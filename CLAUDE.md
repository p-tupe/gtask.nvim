...
- `lua/gtask/config.lua`: Configuration management with defaults and user overrides (proxy URL, token storage, markdown directory)
...
**Key functions**:
- `validate_markdown_dir()`: Checks if configured directory exists and is accessible
- `find_markdown_files()`: Recursively scans directory using `vim.loop.fs_scandir` for `.md` files
- `read_markdown_file()`: Reads file contents into line array
- `parse_markdown_file()`: Combines file reading + parsing, returns tasks with file metadata
- `parse_all_markdown_files()`: Entry point for directory sync, returns array of `{file_path, file_name, tasks}` structures
...
1. **Parse & Group** (`sync_directory_with_google`):
   - Validate markdown directory via `files.validate_markdown_dir()`
   - Scan for `.md` files with `files.find_markdown_files()`
   - Parse all files with `files.parse_all_markdown_files()` (extracts H1 headings and tasks)
   - Group tasks by H1 heading (list name) from each file
   - Add source file metadata to each task

2. **Sync Each List** (`sync_multiple_lists` + `sync_single_list`):
   - For each list name found in markdown:
...
```lua
require('gtask').setup({
  proxy_url = "https://your-deployed-proxy.example.com",  -- Default: "http://localhost:3000"
  token_file = "my_custom_tokens.json",                    -- Default: "gtask_tokens.json"
...
- Token file changes affect where `store.lua` persists OAuth tokens
